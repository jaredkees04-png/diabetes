...
A personal, on-device diabetes management tool built with Flutter. Five
bottom tabs, plus Settings off a gear icon in the app bar:

- **Home** — a "Today" / "History" toggle at the top switches the whole
  tab between two views:
  - **Today** — a quick-glance dashboard: your latest glucose reading
    (flagged if high/low), today's basal and bolus totals,
    readings/average/high-low counts for today, and a merged
    recent-activity feed of your basal, bolus, and glucose logs.
  - **History** — browse your logs by day, week, or month. Each period
    shows aggregate stats (total basal, total bolus, reading
    count/average, high/low count). Day view lists every entry logged
    that day; Week and Month views show a day-by-day breakdown you can
    tap into to jump straight to that day. Step forward/back a period at
    a time, jump back to today, or use the date picker to go straight to
    any day.

  Nothing here is stored separately — both views are computed on the fly
  from the same logs the Basal, Bolus, and Glucose screens write to.
...
- **Settings** (gear icon, top right of the app bar) — insulin-to-carb
  ratio, correction factor, target glucose, and dose-rounding increment
  (0.1 / 0.5 / 1 unit).
